...
public class JhoveBase
{
	...
	if (_configFile != null) {
        ...
        try {
        	config = new InputSource( new FileInputStream ( configFileFile ) );
        }
        catch (IOException e) {
    		// Pick up default configuration from inside the JAR:
    		URL jhoveConf = this.getClass().getResource("jhove-withTextMD.conf");
    		try {
    			config = new InputSource(jhoveConf.openStream() );
    			err = null;
    		} catch ( Exception e2) {
                ...
                config = null;
                throw new
                JhoveException ("Cannot read configuration resource: " +
                  jhoveConf);
    		}
    		/*
                  throw new
